...
# Getting the directory name - where the JSON file are
if len(sys.argv) > 1:
...
for filename in sorted(os.listdir(directory)):
    ...
    # get data  
    with open(directory + '/' + filename) as json_bike_data: # looks at files in file timestamp order
        ...
        for each_station in bike_data:
            ...
            if (each_station['last_update']/1000) > max_timestamp: # dividing by 1000 converts time stamp into milliseconds (conventional)
                
                with connection.cursor() as cursor:
                    # Create a new record
                    sql = "INSERT INTO `StationsStaticTestCopy` (`station`, `name`, `address`,`lat`, `lng`, `banking`, `bonus`, `contract_name`, `bike_stands`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
